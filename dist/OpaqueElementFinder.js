(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : global.OpaqueElementFinder = factory();
})(this, function () {
	'use strict';
  ...
  */
   ...
   */
		this.elementIsTransparentAtPoint = function (element, x, y, alphaTolerance) {
			...
			if (elementsBackgroundIsTransparent(element, alphaTolerance)) {
				...
				return element.nodeName !== 'IMG' || this.imageIsTransparentAtPoint(element, x, y, alphaTolerance);
			}
			return false;
		};
		...
		function getRgbAlphaLevel(colorString) {
			...
			return matches ? normalizeAlphaLevel(matches[1]) : 255; // no RGBA but RGB
		}
   ...
   */
		...
		function normalizeAlphaLevel(value) {
			if (value === lastAlphaLevelInUse) {
				// Optimization as often called with same value.
				return value;
			}
			if (value === undefined) {
			...
			switch (typeof value) {
				...
				case 'number':
					if (!(value <= 255 && value >= 0)) {
						throw new Error('expected a number between 0 and 255 or a ' + 'string representing an opacity between "0" and "1", e.g. "0.25"');
					}
					break;
			}
...
//# sourceMappingURL=OpaqueElementFinder.js.map